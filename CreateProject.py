...
def get_fabric_versions():
    console.print("[cyan]🔍 Поиск версий Minecraft...[/]")
    try:
        ...
        console.print("[red]⚠️ Ошибка сети. Использую оффлайн-список.[/]")
        return ["1.21.11", "1.21.4", "1.21.1", "1.20.1"]

def get_yarn_version(mc_version):
    console.print(f"[cyan]🔍 Поиск Yarn для {mc_version}...[/]")
    try:
        ...
        if data:
            ...
            console.print(f"[green]✅ Найдeн: {latest}[/]")
            return latest
    except: pass
    console.print(f"[yellow]⚠️ Yarn не найден (будет использован Official)[/]")
    return None
...
def download_template(target_folder):
    ...
    try:
        with Progress() as progress:
            task = progress.add_task("[green]Скачивание шаблона...", total=None)
            ...
            with open(zip_path, 'wb') as f:
                for chunk in r.iter_content(chunk_size=8192):
                    f.write(chunk)
            progress.update(task, completed=100, description="[green]Распаковка...")
...
def update_gradle_wrapper(project_path):
    # Gradle 8.14 (Требование Loom 1.11+)
    ...
    console.print("[green]✅ Gradle Wrapper -> 8.14[/]")

def update_loom_version(project_path):
    # Loom 1.9-SNAPSHOT (Для Gradle 8.x)
    ...
    console.print("[green]✅ Fabric Loom -> 1.9-SNAPSHOT[/]")
...
def configure_gradle_properties(project_path, mc_ver, yarn_ver, map_type, mod_ver, group, loader_ver):
    ...
    # Loader берем из аргумента (0.18.4)
    content = f"""
...
"""
    ...
    console.print(f"[green]✅ Config настроен: Loader {loader_ver} | {map_type}[/]")
...
def refactor_java_package(project_path, new_package):
    ...
    console.print(f"[green]✅ Пакеты обновлены: {new_package}[/]")
...
def run():
    ...
    # 2. Loader Version (АВТОМАТИЧЕСКИ ПРЕДЛАГАЕМ 0.18.4)
    ...
    if map_type == "yarn":
        ...
        if not yarn_ver:
            console.print("[yellow]⚠️ Yarn не найден, использую Official[/]")
            map_type = "official"
    ...
    if os.path.exists(project_folder):
        console.print("[red]❌ Папка занята![/]")
        time.sleep(1)
        return
    ...
    if download_template(project_folder):
        # Передаем правильный loader_version (0.18.4)
        ...
        console.print(f"\n[bold green]🎉 Проект {mod_id} создан![/]")
        
        if Prompt.ask("Открыть сейчас?", choices=["y", "n"], default="y") == "y":
            import MainProject
